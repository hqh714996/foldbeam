...
def load_factor_map(path: Path, *, required: bool = True) -> dict[str, Any]:
    if not path.is_file():
        if required:
            raise FileNotFoundError(f"Missing factor map: {path}. Run compile_maps_classify.py first.")
...
def _path_vectors(
    ...
    factor_index = {str(item.get("factor_id", f"F{i + 1}")): i for i, item in enumerate(factors) if isinstance(item, dict)}
    ...
    for feature, op, _threshold in path:
        ...
        entry = (features_cfg.get(str(feature_name)) if feature_name is not None else None) or features_cfg.get(str(feature)) or features_cfg.get(f"f{feature}")
        ...
        if direction_name == "positive_class":
            semantic_direction = 1.0
        elif direction_name == "negative_class":
            semantic_direction = -1.0
        elif direction_name == "higher_target":
            semantic_direction = 1.0
