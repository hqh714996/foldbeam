...
class Config:
    ...
    def validate(self) -> None:
        ...
        grid = graph.get("laplace_alpha_grid")
        if grid is not None:
            if not isinstance(grid, list) or not grid:
                raise ConfigError("laplace_alpha_grid must be a non-empty list.")
            if any(float(value) <= 0.0 for value in grid):
                raise ConfigError("laplace_alpha_grid values must be positive.")
        ...
        search_metric = graph.get("search_metric", "logloss")
        if search_metric not in {"logloss", "balanced_accuracy", "accuracy"}:
            raise ConfigError(f"search_metric must be one of logloss/balanced_accuracy/accuracy, got {search_metric!r}.")
        final_metric = graph.get("final_selection_metric", "balanced_accuracy")
        if final_metric not in {"balanced_accuracy", "accuracy", "logloss", "auc"}:
            raise ConfigError(f"final_selection_metric invalid: {final_metric!r}.")
        gain_metric = graph.get("gain_metric", "gini")
        if gain_metric not in {"gini", "entropy"}:
            raise ConfigError(f"gain_metric must be gini or entropy, got {gain_metric!r}.")
        th_grid = graph.get("threshold_grid")
        if th_grid is not None:
            if not isinstance(th_grid, list) or not th_grid:
                raise ConfigError("threshold_grid must be a non-empty list.")
            if any(not 0.0 < float(v) < 1.0 for v in th_grid):
                raise ConfigError("threshold_grid values must lie in (0,1).")
