...
class Graph:
    ...
    def add_new(self, edge: tuple[int, str], predicate: Predicate) -> None:
        node_id, branch = edge
        if branch != "self":
            raise ValueError("FoldBeam expands open nodes only.")
        node = self.nodes[int(node_id)]
        if not node.is_open:
            raise ValueError("NEW can only expand an open node.")
        left_id = self.next_id()
        right_id = left_id + 1
        self.nodes[int(node_id)] = Node(int(node_id), "internal", feature=int(predicate.feature), threshold=float(predicate.threshold), left=left_id, right=right_id)
    ...
    def add_reuse(self, edge: tuple[int, str], target_id: int) -> None:
        node_id, branch = edge
        if branch != "self":
            raise ValueError("FoldBeam reuses from open nodes only.")
    ...
    def stop(self, edge: tuple[int, str]) -> None:
        node_id, branch = edge
        if branch != "self":
            raise ValueError("FoldBeam stops open nodes only.")
        node_id = int(node_id)
    ...
    def fit_terminals(self, X: np.ndarray, y: np.ndarray, alpha: float) -> None:
        ...
        global_pos_rate = float(np.mean(y)) if len(y) else 0.5
        for node_id in self.reachable_ids():
            node = self.nodes[node_id]
            ...
            mask = leaves == node_id
            n = int(np.sum(mask))
            n_pos = int(np.sum(y[mask])) if n else 0
            node.n = n
            node.n_pos = n_pos
            node.prob = float((n_pos + float(alpha) * global_pos_rate) / (n + float(alpha))) if (n + float(alpha)) > 0 else global_pos_rate

    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        leaves = self.route(X)
        return np.asarray([self.nodes[int(nid)].prob for nid in leaves], dtype=np.float64)
    ...
    def canonical_hash(self) -> str:
        ...
        for node_id in self.reachable_ids():
            node = self.nodes[node_id]
            payload.append((node.node_id, node.kind, node.feature, None if node.threshold is None else round(float(node.threshold), 12), node.left, node.right, round(float(node.prob), 12), int(node.n), int(node.n_pos)))
